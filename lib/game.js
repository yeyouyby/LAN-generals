...
class Game {
  ...
  _tryGenMap(density, cityCount) {
    ...
    while (mountainCount < mountainTarget && frontier.length) {
      ...
      frontier.push(next);
      mountainCount++;
      // 偶尔从旧种子重新开始，避免山脉全都长成细长通道。
      if (Math.random() < 0.18) frontier.push(seeds[rand(seeds.length)]);
    }
    ...
    for (let p = 0; p < count; p++) {
      ...
      for (let t = 0; t < 180; t++) {
        ...
        const score = generals.length ? Math.min(...generals.map((g) => {
          return Math.abs((g % w) - x) + Math.abs(((g / w) | 0) - y);
        })) : Math.min(x, y, w - 1 - x, h - 1 - y) + rand(3);
        if (score > bestScore) { best = i; bestScore = score; }
      }
      if (best < 0 || (generals.length && bestScore < minDist - 1)) return null;
      generals.push(best);
    }
